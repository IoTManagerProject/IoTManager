...
void routerConnect()
{
  ...
  jsonReadStr(settingsFlashJson, "routerpass", _passwordList);
  if (_ssidList.size() > 1)
    triesOne = TRIES;

  if (_ssidList.size() == 0 && _ssidList[0] == "" && _passwordList[0] == "")
  {
    WiFi.begin();
  }
  else
  {
    ...
    {
      _password = _ssid + _passwordList[i].as<String>() + "; ";
    }
